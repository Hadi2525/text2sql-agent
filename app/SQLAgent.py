...
from utils import get_schema, execute_query
...
SQL_QUERY_LIMIT = int(os.getenv("SQL_QUERY_LIMIT", 1000))
...
class SQLAgent:
    ...
    def parse_question(self, state: dict) -> dict:
        ...
        db_path = UPLOAD_DIR / f"{state['uuid']}.sqlite"
        schema = get_schema(db_path).get('schema', '')
        ...
        prompt = ChatPromptTemplate.from_messages([
            ("system", '''You are a data analyst that can help summarize SQL tables and parse user questions about a database. 
                            ...
                            for example, the columns ["Ministry", "Programs", "Department", "Services"] contain nouns and relevant information that explains the question 
                            "What are the costs of some of the programs from different ministries?", but the column "Ministry ID" is not relevant because it does not contain a noun.
                            Do not include columns that contain numbers or dates or IDs.
                        '''),
    ...
    def get_unique_nouns(self, state: dict) -> dict:
        ...
        for table_info in parsed_question['relevant_tables']:
            ...
            if noun_columns:
                ...
                db_path = UPLOAD_DIR / f"{state['uuid']}.sqlite"
                results = execute_query(db_path, query).get('results', [])
                for row in results:
    ...
    def validate_and_fix_sql(self, state: dict) -> dict:
        ...
        prompt = ChatPromptTemplate.from_messages([
            ("system", r'''
                                ...
                                5. Make sure that the SQL query result is not tool large. The whole context should not exceed more than 80000 tokens. Cut the results if it exceeds the limit.
